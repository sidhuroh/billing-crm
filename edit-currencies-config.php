...
<div style="background: #fff;">
    <div id="margin-setter2">
        <div style="padding: 20px;">
            <f style='font-size: 18px; font-weight: 700; color: #555;'>Edit Currency</f>
        </div>
    </div>
            ...
            <div style='padding: 20px;'>
                ...
                if ($update_btn) {
                    ...
                    if ($currency_symbol && $currency_title) {
                        $sql = "UPDATE `currencies` SET `symbol`='$currency_symbol',`title`='$currency_title' WHERE id='$currency_id'";
                        $query = mysqli_query($conn, $sql);
                        echo "<meta http-equiv=\"refresh\" content=\"0; url=edit-currencies-config.php?id=$currency_id&&updated=1\">";
                    } else {
                        echo "<meta http-equiv=\"refresh\" content=\"0; url=edit-currencies-config.php?id=$currency_id&&updated=0\">";
                    }
                }
                $report = @$_GET['updated'];
                if ($report == "1") {
